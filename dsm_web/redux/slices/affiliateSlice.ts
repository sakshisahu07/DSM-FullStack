...
import { updateCredentials } from './authSlice';
...
export const verifyAffiliateOtp = createAsyncThunk(
  'affiliate/verifyOtp',
  async (payload: { phone: string; otp: string }, { rejectWithValue, dispatch }) => {
    try {
      const response = await fetch(`${BASE_URL}/affiliate/verify-otp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Update credentials inside authSlice with new role (AFFILIATE) and token
      if (data.data?.token) {
...
export const registerAffiliate = createAsyncThunk(
  ...
  async (formData: FormData, { rejectWithValue, getState }) => {
    try {
      ...
      // If registration assigns a new token (e.g. for AFFILIATE role), update auth state
      if (data.data?.token) {
        const { dispatch } = require('../store').store;
        dispatch(updateCredentials({ user: data.data.user, token: data.data.token }));
      }
...
const affiliateSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(verifyAffiliateOtp.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(verifyAffiliateOtp.fulfilled, (state) => {
        state.loading = false;
        state.otpVerified = true;
      })
      .addCase(verifyAffiliateOtp.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string;
      })
