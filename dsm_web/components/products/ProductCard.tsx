...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <div
      ...
      className="w-full h-full bg-white rounded-[20px] md:rounded-3xl p-0 relative flex flex-col group hover:shadow-md transition-all duration-300 border border-gray-100 overflow-hidden cursor-pointer"
    >
      {/* Hot Badge (Diagonal Ribbon) */}
      {product.isHot && (
        <div className="absolute top-0 left-0 w-20 h-20 md:w-24 md:h-24 overflow-hidden z-20 pointer-events-none">
      ...
      {/* Discount Badge */}
      {product.discount && !product.isHot && !timeLeftStr && (
        <div className="absolute top-4 left-4 z-20">
          <div className="bg-gradient-to-r from-[#EE9C24] to-[#B3520A] text-white backdrop-blur-sm border border-[#EE9C24] text-[0.6rem] md:text-[0.8rem] px-3 md:px-6 py-1 md:py-1.5 flex items-center gap-2 rounded-lg shadow-lg">
            <span>{typeof product.discount === 'number' ? `${product.discount}% Off` : product.discount}</span>
          </div>
        </div>
      )}

      {/* Flash Sale Badge */}
      {timeLeftStr && !product.isHot && (
        <div className="absolute top-4 left-4 z-20">
      ...
      <div className="relative w-full aspect-[4/3] bg-white p-2 md:p-6 flex items-center justify-center overflow-hidden">
        {/* Main Image */}
        <Image
          src={!product.image || product.image === "false" || product.image === "null" ? "/bluetooth.png" : product.image}
          alt={name}
          width={280}
          height={180}
        ...
        {images.length > 1 && (
          <Image
            src={(!images[1] || images[1] === "false" || images[1] === "null") ? (images[0] === "false" || images[0] === "null" ? "/bluetooth.png" : images[0]) : images[1]}
            alt={`${name} hover`}
            width={280}
            height={180}
      ...
      <div className="p-3 md:p-6 pt-1 md:pt-2 flex-1 flex flex-col space-y-2 md:space-y-4 border-t border-gray-50">
        ...
        <div className="flex flex-wrap gap-1 md:gap-2">
          <span className="text-[8px] md:text-[8px] font-bold text-gray-500 border border-gray-100 px-1.5 py-0.5 md:px-1 md:py-1 rounded-[4px] md:rounded-md bg-gray-50/50">
        ...
        <div className="flex items-center justify-between pt-1 md:pt-2 mt-auto">
          <div className="flex flex-col">
            <span className="text-[8px] md:text-[8px] text-gray-400 font-bold uppercase tracking-tighter">Price</span>
