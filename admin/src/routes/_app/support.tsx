...
function mapApiTicket(raw: any): Ticket {
  ...
  const userName = raw.user?.name || raw.userName || raw.name || "Unknown";
  const userPhone = raw.user?.phone || raw.userPhone || raw.phone || "";
  const avatar = userName.split(" ").map((w: string) => w[0]).join("").slice(0, 2).toUpperCase();

  // Map messages/replies
  const rawMessages: any[] = raw.messages || raw.replies_list || [];
  const messages: Message[] = rawMessages.map((m: any) => ({
    role: m.role === "support" || m.sender === "admin" ? "support" as const :
      m.role === "original" ? "original" as const : "user" as const,
    text: m.text || m.message || m.content || "",
    timestamp: m.timestamp || m.createdAt || "",
  }));

  // If there are no messages, create an original message from subject
  if (messages.length === 0 && (raw.subject || raw.message)) {
    messages.push({
      role: "original",
      text: raw.subject || raw.message || "",
      timestamp: `${date} ${time}`,
    });
  }
  ...
  return {
    ...
    subject: raw.subject || raw.message || raw.title || "No subject",
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem className="text-xs font-bold">Mark as Resolved</DropdownMenuItem>
                              <DropdownMenuItem className="text-xs font-bold">Assign to Agent</DropdownMenuItem>
                              <DropdownMenuItem className="text-xs font-bold text-rose-500">Delete Ticket</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </div>
                      </td>
