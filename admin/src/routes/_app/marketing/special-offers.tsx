...
export const Route = createFileRoute("/_app/marketing/special-offers")({
...
/* ─── Helper: map raw API special-offer → Sale ─────────────────── */
function mapSpecialOffer(
  fs: any,
...
function SpecialOffersPage() {
    ...
    try {
      ...
      try {
        const res = await apiFetch(
          `${API_BASE}/special-offers/all?page=1&limit=50`
        );
        const json = await res.json();
        if (json.success) {
          salesArray =
            json.data?.data ??
            json.data?.specialOffers ??
            (Array.isArray(json.data) ? json.data : []);
        } else {
          const fallbackRes = await apiFetch(
            `${API_BASE}/special-offer?page=1&limit=50`
          );
          const fallbackJson = await fallbackRes.json();
          if (fallbackJson.success) {
            salesArray =
              fallbackJson.data?.data ??
              fallbackJson.data?.specialOffers ??
      ...
      setItems(
        salesArray.map((fs: any) =>
          mapSpecialOffer(fs, productsRef.current, combosRef.current)
        )
      );
    } catch (err) {
  ...
  const onDelete = async (s: Sale) => {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/special-offer/${s.id}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  const toggle = async (id: string) => {
    ...
    try {
      const res = await apiFetch(`${API_BASE}/special-offer/${id}`, {
        method: "PATCH",
      ...
      if (json.success) {
        toast.success(
          json.message ??
            `Special offer ${willBeActive ? "activated" : "deactivated"}`
        );
      } else {
        // Revert on failure
        setItems((prev) =>
      ...
      toast.error(`Failed to ${willBeActive ? "activate" : "deactivate"} Special offer`);
    }
  };
  ...
  const handleSubmit = async (v: SaleFormData) => {
    ...
    if (editing) {
      ...
      try {
        const res = await apiFetch(`${API_BASE}/special-offer/${editing.id}`, {
          method: "PATCH",
          body: JSON.stringify({
            title: v.title,
        ...
        console.warn("PATCH /special-offer/:id update failed", e);
      }

      // 2) Add / replace items via PATCH add-items
      try {
        const res = await apiFetch(
          `${API_BASE}/special-offer/${editing.id}/add-items`,
          {
            method: "PATCH",
            body: JSON.stringify({
        ...
        console.warn("PATCH /special-offer/:id/add-items failed", e);
      }
      ...
      /* ── CREATE mode ── */
      try {
        const res = await apiFetch(`${API_BASE}/special-offer`, {
          method: "POST",
          body: JSON.stringify({
            title: v.title,
            discountType: v.discountType,
        ...
        if (json.success) {
          toast.success(json.message ?? "Special offer created");

          // If user wanted it inactive, deactivate right after creation
          const newId = json.data?._id;
          if (newId && !v.isActive) {
            try {
              await apiFetch(`${API_BASE}/special-offer/${newId}`, {
                method: "PATCH",
          ...
          toast.error(json.message ?? "Failed to create Special offer");
        }
      } catch {
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <Button className="gap-1.5" onClick={onCreate}>
            <Plus className="h-4 w-4" /> New Special offer
          </Button>
        }
      />
      ...
      {loading ? (
        ...
        <Card className="p-20 text-center text-muted-foreground italic bg-card shadow-sm border">
          No Special offers found. Create your first one!
        </Card>
      ) : (
      ...
      <FormDialog<SaleFormData>
        ...
        title={editing ? "Edit Special offer" : "New Special offer"}
        fields={fields}
        initialValues={
          editing
            ? {
