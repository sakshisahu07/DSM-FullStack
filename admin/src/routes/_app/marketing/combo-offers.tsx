...
function ComboDrawer({ open, onOpenChange, combo, onSave }: {
  ...
  useEffect(() => {
    if (open) {
      if (combo) {
        ...
        setCountries(combo.countries || []);
        setStates(combo.states || []);
        setCities(combo.cities || []);
        setPincodes(combo.pincodes || []);
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Stringify complex arrays as the API seems to expect them as JSON text based on the Postman screenshot
            ...
            <TabsContent value="basic" className="space-y-4 pt-0 mt-0">
              ...
              <div className="space-y-2">
                ...
                <div className="text-[10px] bg-muted/50 p-3 rounded-lg border border-dashed text-muted-foreground">
                   Enter country/state/city IDs to restrict this combo to specific regions. Leave empty for nationwide availability.
                </div>
                {/* Simplified for now as full location management is complex */}
                <div className="grid grid-cols-2 gap-3">
                   <div className="space-y-1">
                      <Label className="text-[10px]">Country IDs</Label>
                      <Input value={JSON.stringify(countries)} onChange={(e) => {try{setCountries(JSON.parse(e.target.value))}catch(e){}}} className="text-[10px]" />
                   </div>
                   <div className="space-y-1">
                      <Label className="text-[10px]">Pincodes</Label>
                      <Input value={JSON.stringify(pincodes)} onChange={(e) => {try{setPincodes(JSON.parse(e.target.value))}catch(e){}}} className="text-[10px]" />
                   </div>
                </div>
              </div>
            </TabsContent>
