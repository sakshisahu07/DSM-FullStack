...
export default class SpecialOfferController {
  ...
  static async getSpecialOfferProducts(req, res) {
    return handleApiRequest(req, res, async () => {
      const result = await SpecialOfferService.getSpecialOfferProducts(req.query);
      return [{ data: result }, "Filtered special offer products fetched successfully"];
    });
  }
