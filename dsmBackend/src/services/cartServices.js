...
export default class CartService {
  ...
  // ─── GET CART ─────────────────────────────────────────────────
  static async getCart(userId, destPincode = null) {
    const cacheKey = `cart:${userId}:pin:${destPincode || 'none'}`;
    ...
    const summary = await calculateCartSummary(cart.items, coupon, destPincode);
  ...
  static async _clearCartCache(userId) {
    try {
      const keys = await redisClient.keys(`cart:${userId}:pin:*`);
      ...
      console.error("[Cart Cache Clear Error]", err);
    }
  }
}
