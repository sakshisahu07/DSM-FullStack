...
export default class ComboService {
  static async createCombo(payload) {
    ...
    for (const item of items) {
      ...
      if (variant.brand) brandSet.add(variant.brand.toString());
    }
    ...
    const combo = await comboModel.create({
      ...
      stock: minStock,
      categories: payload.categories && payload.categories.length > 0 ? payload.categories : [...categorySet],
      subCategories: payload.subCategories && payload.subCategories.length > 0 ? payload.subCategories : [...subCategorySet],
      brands: payload.brands && payload.brands.length > 0 ? payload.brands : [...brandSet],
    });
  ...
  static buildMatch(filters) {
    ...
    if (brand && mongoose.Types.ObjectId.isValid(brand)) {
      match.brands = { $in: [new mongoose.Types.ObjectId(brand)] };
    }

    if (minPrice !== undefined || maxPrice !== undefined) {
      match.comboPrice = {};
      if (minPrice !== undefined && minPrice !== "") {
        match.comboPrice.$gte = parseFloat(minPrice);
      }
      if (maxPrice !== undefined && maxPrice !== "") {
        match.comboPrice.$lte = parseFloat(maxPrice);
      }
    }
  ...
  static async updateCombo(id, payload) {
    ...
    if (payload.items) {
      ...
      payload.categories = payload.categories && payload.categories.length > 0 ? payload.categories : [...categorySet];
      payload.subCategories = payload.subCategories && payload.subCategories.length > 0 ? payload.subCategories : [...subCategorySet];
      payload.brands = payload.brands && payload.brands.length > 0 ? payload.brands : [...brandSet];
    }
