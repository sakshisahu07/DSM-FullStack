...
export default class AffiliateService {
  ...
  static async verifyOtp(phone, otp) {
    console.log(`[Affiliate.verifyOtp] phone=${phone}, otp=${otp}`);
    const user = await userModel.findOne({ number: phone }).select("_id otp role disable");
    console.log(`[Affiliate.verifyOtp] user found:`, user ? user._id : "NOT FOUND");
    if (!user) throw new AppError("Phone number not found. Please send OTP first.", 404);
    console.log(`[Affiliate.verifyOtp] stored OTP:`, user.otp?.code, "expiresAt:", user.otp?.expiresAt);
    if (!user.otp?.code) throw new AppError("OTP not sent. Please request OTP first.", 400);
    if (new Date() > user.otp.expiresAt) throw new AppError("OTP has expired. Please request again.", 400);
    if (user.otp.code !== otp) throw new AppError(`Invalid OTP. Expected ${user.otp.code}, got ${otp}`, 400);
  ...
  static async registerAffiliate(userId, payload, files) {
    const user = await userModel.findById(userId).select("_id disable number role").populate("role").lean();
    if (!user) throw new AppError("User not found", 404);
    if (user.disable) throw new AppError("Account is disabled", 403);

    const isDev = process.env.NODE_ENV !== "production";
    const DEV_PLACEHOLDER = "https://via.placeholder.com/400x250?text=KYC+DOC";

    const panImage   = files?.panImage?.[0]?.location   || (isDev ? DEV_PLACEHOLDER : null);
    const adharImage = files?.adharImage?.[0]?.location || (isDev ? DEV_PLACEHOLDER : null);
    ...
    // Only change role to AFFILIATE if they are a regular USER or have no role
    // This prevents ADMINs from losing admin privileges when registering as affiliate
    let currentRoleName = user.role?.name || "USER";
    if (currentRoleName === "USER") {
      await userModel.updateOne({ _id: user._id }, { role: affiliateRole._id });
      user.role = affiliateRole; // update local object for JWT payload
    }
    ...
    return { affiliate, token, user: { ...user, role: user.role } };
  }
