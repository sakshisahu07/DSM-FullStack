...
function CategoriesPage() {
  ...
  const onEdit = useCallback((c: Category) => { setEditing(c); setOpen(true); }, []);
  ...
  const onToggleDisable = useCallback(async (id: string, currentDisable: boolean | undefined) => {
    try {
      // Optimistic update
      setItems(prev => prev.map(c => c._id === id ? { ...c, disable: !currentDisable } : c));
      const res = await fetch(`${API_BASE}/category/${id}/status`, {
        method: "PATCH",
        headers: getAuthHeaders()
      });
      const json = await res.json();
...
const CategoryCardItem = memo(({ c, onEdit, onDeleteClick, onToggleDisable }: { 
  c: Category, 
  ...
  const handleToggle = async () => {
    setToggling(true);
    await onToggleDisable(c._id, c.disable);
    setToggling(false);
  };
