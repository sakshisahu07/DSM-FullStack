...
const HeroSection = ({ categories: propCategories, loading: propLoading }: { categories?: any[], loading?: boolean }) => {
  ...
  const categories = categoriesList.length > 0 ? categoriesList.map(c => ({
    ...
    { name: 'Raspberry Pi', items: '120 Items', icon: '/navImg.png' },
    { name: 'Motors', items: '130 Items', icon: '/navImg.png' },
    { name: 'Sensors', items: '105 Items', icon: '/navImg.png' },
    { name: 'Robot Parts', items: '130 Items', icon: '/navImg.png' },
    { name: 'Robot Parts', items: '125 Items', icon: '/navImg.png' },
    { name: 'Development Boards', items: '120 Items', icon: '/navImg.png' },
    { name: 'Programmers', items: '125 Items', icon: '/navImg.png' },
  ];
            ...
            {categoriesLoading ? (
              ...
              categories.map((cat, idx) => (
                <SwiperSlide key={idx}>
                  ...
                  >
                    <div className="w-[50px] h-[50px] md:w-20 md:h-20 rounded-full bg-[#FDF4EE] flex items-center justify-center mb-1 md:mb-3 overflow-hidden">
                      <Image
                        src={cat.icon}
                        alt={cat.name}
                        width={80}
                    ...
                    <div className="flex flex-col items-center w-full px-1">
                      <span className="text-[8px] md:text-[11px] font-bold text-gray-800 text-center leading-[1.1] md:leading-tight line-clamp-1 md:line-clamp-none md:whitespace-nowrap">{cat.name}</span>
