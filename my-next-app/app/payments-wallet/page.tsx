                            ...
                            <div>
                              <div className="flex items-center gap-3">
                                <p className="text-sm font-semibold text-[#222222] capitalize">
                                  {txn.walletPurpose?.toLowerCase().replace('_', ' ') || 'Transaction'}
                                </p>
                                <span
                                  className={`text-[11px] px-2 py-0.5 rounded capitalize ${
                                    txn.walletType === 'CREDIT' ? 'bg-[#34C7591A] text-[#34C759]' : 'bg-[#FF3B301A] text-[#FF3B30]'
                                  }`}
                                >
                                  {txn.paymentGateway === 'RAZORPAY' ? 'Added from Bank' : txn.paymentMethod?.toLowerCase() || 'Wallet'}
                                </span>
                              </div>
                          ...
                          <div className="text-right flex items-center gap-1.5">
                            <span
                              className={`text-sm font-medium ${
                                txn.walletType === 'CREDIT' ? 'text-[#34C759]' : 'text-[#222222]'
                              }`}
                            >
                              {txn.walletType === 'CREDIT' ? 'Credit' : 'Debit'}
                            </span>
                            <span
                              className={`text-base font-bold ${
                                txn.walletType === 'CREDIT' ? 'text-[#34C759]' : 'text-[#FF3B30]'
                              }`}
                            >
                              {txn.walletType === 'CREDIT' ? '+' : '-'}₹{txn.amount}
                            </span>
                          </div>
                        </div>
                      ))
                    ...
                    <div key={txn._id || `txn-mobile-${idx}`} className="bg-white rounded-3xl p-4 border border-gray-100 shadow-sm flex items-center justify-between gap-3 group active:bg-orange-50/10 transition-colors">
                      ...
                      <div className="text-right">
                        <span className={`inline-block px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider mb-1.5 ${
                          txn.walletType === 'CREDIT' ? 'bg-[#E8F8EE] text-[#34C759]' : 'bg-[#FFF0F0] text-[#FF3B30]'
                        }`}>
                          {txn.paymentGateway === 'RAZORPAY' ? 'Bank Account' : (txn.walletPurpose === 'REFERRAL' ? 'Referral' : (txn.walletType === 'CREDIT' ? 'Earned' : 'Wallet'))}
                        </span>
                        <p className={`text-sm font-black ${txn.walletType === 'CREDIT' ? 'text-[#34C759]' : 'text-[#FF3B30]'}`}>
                          {txn.walletType === 'CREDIT' ? '+' : '-'}₹{txn.amount}
                        </p>
                      </div>
                    </div>
