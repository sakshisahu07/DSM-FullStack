...
import { fetchAffiliateDashboard } from '@/redux/slices/affiliateSlice';
...
export default function AffiliateDashboardPage() {
  ...
  const { dashboardData, loading, error } = useSelector((state: RootState) => state.affiliate);
  ...
  const hasPendingApplication = dashboardData && 
                                (dashboardData.status === 'pending' || dashboardData.status === 'rejected');

  // We show the form if we have no data, if there's an error, or if the account isn't active yet
  const showRegistrationForm = !isActiveAffiliate || !!error;
  ...
  if (showRegistrationForm) {
    return (
      <DashboardShell 
        title="Become a DSM Affiliate" 
        subtitle="Complete your KYC to start earning commissions"
      >
        <div className="relative overflow-hidden">
           <AffiliateRegistrationForm 
              initialView={hasPendingApplication ? 'success' : 'landing'} 
              isAlreadyReview={dashboardData?.status === 'pending'} 
           />
        </div>
      </DashboardShell>
    );
